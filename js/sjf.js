function SJF(num, mat) {
    ...
    for (let i = 1; i < rows.length; i++) {
        rows[i].children[0].innerHTML = '<input id="spid' + i + '" placeholder="pid' + i + '" value=' + mat[i - 1][0] + '>';
        rows[i].children[1].innerHTML = '<input id="sat' + i + '" placeholder="at' + i + '" value=' + mat[i - 1][1] + '>';
        rows[i].children[2].innerHTML = '<input id="sbt' + i + '" placeholder="bt' + i + '" value=' + mat[i - 1][2] + '>';
...
function SJFfn() {
    ...
    for (let i = 1; i < SJFcounter; i++) {
        if (document.getElementById('at' + i) !== undefined && document.getElementById('sbt' + i) !== undefined) {
            console.log(i);
            // mat[i][0] = i;
            mat.push([]);
...
function addSJFRow() {
    $('#sjfTable').append('<tr><td><input disabled type="number" id="spid' + SJFcounter + '" placeholder="pid' + SJFcounter + '" value="' + SJFcounter + '"/></td><td><input type="number" id="sat' + SJFcounter + '" placeholder="at' + SJFcounter + '" /></td><td><input type="number" id="sbt' + SJFcounter + '" placeholder="bt' + SJFcounter + '" /></td><td></td><td></td><td></td></tr>');
    SJFcounter += 1;
}
