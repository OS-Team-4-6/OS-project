function rr(pt, at, bt, n, timeQuantum) {
  ...
  remain = n;
  for (count = 0; count < n; count++) rt[count] = bt[count];
  ...
  document.getElementById("att").innerHTML =
    "Average Turn-around Time : " + turnAroundTime / n;
  document.getElementById("awt").innerHTML =
    "Average Waiting Time : " + waitTime / n;
...
function RRfn() {
  ...
  for (let i = 1; i < RRcounter; i++) {
    if (
      document.getElementById("rat" + i) !== undefined &&
      document.getElementById("rbt" + i) !== undefined
    ) {
...
function addRRRow() {
  $("#rrTable").append(
    '<tr><td><input disabled type="number" id="rpid' +
      RRcounter +
      '" placeholder="pid' +
      RRcounter +
      '" value="' +
      RRcounter +
      '" /></td><td><input type="number" id="rat' +
      RRcounter +
      '" placeholder="at' +
      RRcounter +
      '" /></td><td><input type="number" id="rbt' +
      RRcounter +
      '" placeholder="bt' +
      RRcounter +
